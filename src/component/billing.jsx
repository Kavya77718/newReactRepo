...
class Billing extends Component {
  ...
  deleteBill(billingId) {
    BillingService.deleteBill(billingId).then((res) => {
      this.setState({
        bills: this.state.bills.filter((bill) => bill.billingId !== billingId),
      });
    });
  }
  ...
  render() {
    return (
      <div>
        ...
        <div className="row">
          <button className="btn btn-info" onClick={this.addBill}>
            Add Billing
          </button>
        </div>
            ...
            <tbody>
              {this.state.bills.map((bill) => (
                <tr key={bill.billingId}>
                  ...
                  <td>
                    <button
                      onClick={() => this.deleteBill(bill.billingId)}
                    ...
                    <button
                      ...
                      onClick={() => this.viewBill(bill.billingId)}
                    ...
                    <button
                      style={{ marginLeft: "10px" }}
                      onClick={() => this.updateBill(bill.billingId)}
